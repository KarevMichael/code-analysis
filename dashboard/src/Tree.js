...
const File = ({ data }) => {
  ...
  return (
    <>
    {!hide &&
      (<WBox title={data.filePath} onClose={onHide}>
          <LinesChart lines={lines}/>
        </WBox>
      )}
...
const TreeRecursive = ({ data, handleFileClick }) => {
  return data.map((item) => {
    if (item.type === "folder") {
      return (
        <Folder name={item.name} key={item.name}>
          <TreeRecursive data={item.children} handleFileClick={handleFileClick}/>
        </Folder>
      );
    } else {
...
const Tree = ({ data, handleFileClick }) => {
  return (
    <StyledTree>
      <TreeRecursive data={data} handleFileClick={handleFileClick}/>
    </StyledTree>
  );
};
