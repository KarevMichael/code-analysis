...
const { extractStatistics, getReport } = require('./extractors/extractStatistics');
...
const updatedIntersection = intersectedReport.map(updateCandidate => {
	const comparable = extracted.find(({filePath}) => filePath === updateCandidate.filePath);
	return ({
		...comparable,
		...
		statements: [
			...updateCandidate.statements.map((statement, index) => {
				...
				return ({
					...
					complexity: updateComplexity({oldSymbols: statement.symbols, newSymbols: comparableStatement.symbols}).complexity,
					lines: [
						statement.lines.map((line, index) => {
							return {
								complexity: updateComplexity({oldSymbols: line.symbols, newSymbols: comparableStatement.lines[index]?.symbols}).complexity,
...
const differenceReports = [...differenceBy(extracted, report, 'filePath'), ...differenceBy(report, extracted, 'filePath')];
